...
public class ThingsToDoDataSource {
    ...
    public ThingToDo getThingToDo(long id){
        ...
        if(cursor != null){
            cursor.moveToFirst();
            ThingToDo todo = new ThingToDo(cursor.getLong(0), cursor.getString(1), cursor.getInt(2) == 0? false : true);
            cursor.close();
            return todo;
        }
        return null;
    ...
    public List<ThingToDo> getAllThingsToDo(){
        ...
        if(cursor != null){
            cursor.moveToFirst();
            do {
                thingsToDo.add(new ThingToDo(cursor.getLong(0), cursor.getString(1), cursor.getInt(2) == 0? false : true));
